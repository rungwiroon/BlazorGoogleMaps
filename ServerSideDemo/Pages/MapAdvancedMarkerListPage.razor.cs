...
public partial class MapAdvancedMarkerListPage
{
    ...
    private AdvancedMarkerList _markerList;
    ...
    private async Task<IEnumerable<Marker>> GetMarkers(IEnumerable<LatLngLiteral> coords, Map map)
    {
        ...
        {
            var marker = await AdvancedMarkerView.CreateAsync(map1.JsRuntime, new AdvancedMarkerViewOptions()
            {
    ...
    private async Task AddMarkersGroup(IEnumerable<LatLngLiteral> coordinates)
    {
        if (_markerList == null)
        {
            _markerList = await AdvancedMarkerList.CreateAsync(
                map1.JsRuntime,
                coordinates.ToDictionary(s => Guid.NewGuid().ToString(), y => new AdvancedMarkerViewOptions()
                {
        ...
        {
            var cordDic = coordinates.ToDictionary(s => Guid.NewGuid().ToString(), y => new AdvancedMarkerViewOptions()
            {
                Position = new LatLngLiteral() { Lng = y.Lng, Lat = y.Lat },
                Map = map1.InteropObject,
                //Icon = new Icon() { Url = s.MarkerIconPath, ScaledSize = iconSize, Anchor = iconAnchor },
