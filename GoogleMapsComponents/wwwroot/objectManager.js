...
window.googleMapsObjectManager = {
        ...
        obj['extensionFunctions'] = {
            addListenerNoArgument: function (eventName, dotNetObjRef) {
                const mapEventListener = obj.addListener(
                    eventName,
                    () => dotNetObjRef.invokeMethodAsync(`Invoke0`)
                );

                return mapEventListener;
            },

            addListenerWithArgument: function (eventName, dotNetObjRef) {
                const mapEventListener = obj.addListener(
                    eventName,
                    (eventData) => {
                        const stopStatus = dotNetObjRef.invokeMethod(`Invoke1`, eventData);
            ...
            addAsyncListenerWithArgument: function (eventName, dotNetObjRef) {
                const mapEventListener = obj.addListener(
                    eventName,
                    (eventData) => {
    ...
    //invoke2: function (uuid, functionName) {
    //    const obj = window._blazorGoogleMapsObjects[uuid];
    //    //delete window._blazorGoogleMapsObjects[uuid];

    //    //console.dir(window._blazorGoogleMapsObjects);
    //    //console.log(obj.toString());
    //    //console.dir(obj.stop());
    //    //obj.stop();
    //    //console.log("stop called in revoke object function");
    //    obj[functionName]();
